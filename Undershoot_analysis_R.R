...
Master <- read.csv("Control_undershoot.CSV", sep = ",")
...
#T_U:Time (s) to reach the peak undershoot
#U_shoot: raw undershoot value
#U_norm: raw values normalized to DH, ZT3 values
...
#Normalized Undershoot ##############################################################

test_anova_assumptions(data = Master, dv = "U_norm", iv1 = "Region", iv2 = "ZT")
...
set.seed(123)
model_1 <- aovperm(U_norm ~ Region * ZT, data = Master, np = 10000)
anova <- data.frame(summary(model_1))


#                  SS  df         F parametric.P..F. resampled.P..F.
#Region     54.017714   1 26.651858     1.161257e-06          0.0001
#ZT          9.332484   2  2.302282     1.050593e-01          0.1077
#Region:ZT  12.459951   2  3.073814     5.042869e-02          0.0482
#Residuals 212.812932 105        NA               NA              NA
...
auto_fit(Master$U_norm)
#       Distribution   AIC
#gamma          gamma 357.7 #lowest and most appropriate
#exp              exp 360.3
#weibull      weibull 361.7
#norm            norm 423.4
#lnorm          lnorm 487.2
...
summarize_significant_effects(Fit_U) 
...
model_int_emmeans <- emmeans(Fit_U, ~ Region |ZT )
...
model_int_emmeans <- emmeans(Fit_U, ~  ZT|Region )
...
fitted_vals <- fitted(Fit_U, newdata = new_data, re_formula = NA, probs = c(0.025, 0.975))
